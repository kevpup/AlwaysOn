...
export function DraggableBankItem({
  deviceType,
  scenarioId,
  showPreview = true,
  sportCategory,
  widget,
}: DraggableBankItemProps) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`mx-auto w-full max-w-[220px] cursor-grab rounded-2xl border border-border bg-card/80 transition-all active:cursor-grabbing ${
        isDragging
          ? "z-50 scale-[1.02] border-primary/40 opacity-50 shadow-lg"
          : "hover:border-primary/30 hover:bg-secondary/40"
      } ${showPreview ? "p-3" : "px-3 py-2"}`}
    >
      <div className={showPreview ? "mb-3 flex items-center gap-2" : "flex items-center gap-2"}>
